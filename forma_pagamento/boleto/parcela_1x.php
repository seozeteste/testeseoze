...
if ($data['errors']['0']['code'] == '') {
  if ($data['ERROR'] == '') {
    $insere_pedido = mysql_query("UPDATE pedido SET codigo_boleto_1 = '{$data[id]}', url_boleto_1 = '{$data["_links"]["payBoleto"]["redirectHref"]}' WHERE id_pedido = '{$id_pedido}'") or die (mysql_error());
   if ($insere_pedido) {
     ?>
